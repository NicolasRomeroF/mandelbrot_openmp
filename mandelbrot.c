...
profundidad, tasa de muestreo, alto y ancho de la matriz
Salida: Matriz con los valores obtenidos del algoritmo
*/
float** mandelbrot(float infRe, float supRe, float infIm, float supIm, int depth, float sampRate, int width, int heigth){
    int i,j,n;
...
int main(int argc, char **argv){
    ...
    printf("i: %d a: %f b: %f c: %f d: %f s: %f f: %s\n",depth,infRe,infIm,supRe,supIm,sampRate,sName);

    int width = round((supRe-infRe)/sampRate);
    int heigth = round((supIm-infIm)/sampRate);
    printf("width: %d heigth: %d\n",width,heigth);
    float** matrix = mandelbrot(infRe,supRe,infIm,supIm,depth,sampRate,width,heigth);
